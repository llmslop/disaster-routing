...
for _, row in df_deadzone.iterrows():
    ...
    affected_edges = [edge for edge in A if edge[0] in affected_nodes or edge[1] in affected_nodes]
...
def generate_requests(num_requests, V, Z, C):
    ...
    for _ in range(num_requests):
        ...
        zr = max((z for z in Z if any(sr in edge[:2] for edge in z)), key=lambda z: sum(1 for edge in z if sr in edge[:2]), default=[])  # Chọn vùng thảm họa chứa sr
        kr = int(sum(1 for edge in A if edge[0] == sr or edge[1] == sr) / 2)   # Bậc của nút sr
...
# Ví dụ sử dụng:
num_requests = 2  # Số lượng yêu cầu cần tạo
R = generate_requests(num_requests, V, Z, C)
print("Tập hợp R :")
print(R)

# Biến nhị phân
p_kra = {(k, r_idx, a_idx): LpVariable(f"p_{k}_{r_idx}_{a_idx}", cat='Binary') 
                    for r_idx, r in enumerate(R) 
                    for a_idx, a in enumerate(A) 
                    for k in range(1, r['|kr|'] + 1)}

lambda_krd = {(k, r_idx, d): LpVariable(f"lambda_{k}_{r_idx}_{d}", cat='B..inary')
                    for r_idx, r in enumerate(R) 
                    for d in D 
                    for k in range(1, r['|kr|'] + 1)}

R_crd = {(c, r_idx, d): LpVariable(f"R_{c}_{r_idx}_{d}", cat='Binary')
                    for c in C 
                    for r_idx, r in enumerate(R) 
                    for d in D}

w_kr = {(k, r_idx): LpVariable(f"w_{k}_{r_idx}", cat='Binary')
                    for r_idx, r in enumerate(R) 
                    for k in range(1, r['|kr|'] + 1)}

alpha_krz = {(k, r_idx, z_idx): LpVariable(f"alpha_{k}_{r_idx}_{z_idx}", cat='Binary')
                    for r_idx, r in enumerate(R) 
                    for z_idx, z in enumerate(Z)
                    for k in range(1, r['|kr|'] + 1)}

Xi_ir = {(i, r_idx): LpVariable(f"Xi_{i}_{r_idx}", cat='Binary')
                    for r_idx, r in enumerate(R) 
                    for i in range(1, r['|kr|'] + 1)}

beta_kkp_r = {(k, kp, r_idx): LpVariable(f"beta_{k}_{kp}_{r_idx}", cat='Binary')
                    for r_idx, r in enumerate(R) 
                    for k in range(1, r['|kr|'] + 1) 
                    for kp in range(1, r['|kr|'] + 1) } #if k > kp}

beta_kkp_rrp = {(k, kp, r_idx, rp_idx): LpVariable(f"beta_{k}_{kp}_{r_idx}_{rp_idx}", cat='Binary')
                    for r_idx, r in enumerate(R) 
                    for rp_idx, rp in enumerate(R) #if r_idx > rp_idx 
                    for k in range(1, r['|kr|'] + 1) 
                    for kp in range(1, rp['|kr|'] + 1)}

gamma_kkp_r = {(k, kp, r_idx): LpVariable(f"gamma_{k}_{kp}_{r_idx}", cat='Binary')
               for r_idx, r in enumerate(R) 
               for k in range(1, r['|kr|'] + 1) 
               for kp in range(1, r['|kr|'] + 1) } #if k > kp}

gamma_kkp_rrp = {(k, kp, r_idx, rp_idx): LpVariable(f"gamma_{k}_{kp}_{r_idx}_{rp_idx}", cat='Binary')
                 for r_idx, r in enumerate(R) 
                 for rp_idx, rp in enumerate(R) #if r_idx > rp_idx 
                 for k in range(1, r['|kr|'] + 1) 
                 for kp in range(1, rp['|kr|'] + 1)}
...
b_kmr = {(k, m_idx, r_idx): LpVariable(f"b_{k}_{m_idx}_{r_idx}", cat='Binary')
                    for r_idx, r in enumerate(R) 
                    for k in range(1, r['|kr|'] + 1)
...
# Biến nguyên
phi_krm = {(k, r_idx, m_idx): LpVariable(f"phi_krm_{k}_{r_idx}_{m_idx}", lowBound=0, upBound=len(S), cat='Integer')
                    for r_idx, r in enumerate(R) 
                    for m_idx, m in enumerate(M)
                    for k in range(1, r['|kr|'] + 1)}

phi_kra = {(k, r_idx, a_idx): LpVariable(f"phi_kra_{k}_{r_idx}_{a_idx}", lowBound=0, upBound=len(S), cat='Integer')
                    for r_idx, r in enumerate(R) 
                    for a_idx, a in enumerate(A) 
                    for k in range(1, r['|kr|'] + 1)}

phi_kr = {(k, r_idx): LpVariable(f"phi_kr_{k}_{r_idx}", lowBound=0, upBound=len(S), cat='Integer')
                    for r_idx, r in enumerate(R) 
                    for k in range(1, r['|kr|'] + 1)}

g_kr = {(k, r_idx): LpVariable(f"g_kr_{k}_{r_idx}", lowBound=0, upBound=len(S)-1, cat='Integer')
                    for r_idx, r in enumerate(R) 
                    for k in range(1, r['|kr|'] + 1)}
...
for r_idx, r in enumerate(R) :
    for k in range(1, r['|kr|'] + 1):
        ...
        mdl += lpSum(lambda_krd[k, r_idx, d] for d in D) == w_kr[k, r_idx], f"dc_assignment_content_placement_{k}_{r_idx}"
...
for r_idx, r in enumerate(R) :
    for c in C:
        ...
        mdl += lpSum(R_crd[c, r_idx, d] for d in D) == K, f"content_storage_exact_K_{c}_{r_idx}"

# RB(4): Ràng buộc đảm bảo rằng các DC được phân bổ khác nhau cho các đường dẫn
for r_idx, r in enumerate(R) :
    for d in D:
        # Ràng buộc: Σ λ_{k, r, d} ≤ R_{c, r, d}, ∀r, ∀d
        mdl += lpSum(lambda_krd[k, r_idx, d] for k in range(1, r['|kr|'] + 1)) <= R_crd[c, r_idx, d], f"dc_unique_assignment_{r_idx}_{d}"
...
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        for v in V:
            ...
            if v == r['sr']:
                mdl += (outgoing_flow - incoming_flow) == w_kr[k, r_idx], f"flow_conservation_source_{k}_{r_idx}"
            elif v in D:
                mdl += (outgoing_flow - incoming_flow) == -lambda_krd[k, r_idx, v], f"flow_conservation_dc_{k}_{r_idx}_{v}"
            else:
                mdl += (outgoing_flow - incoming_flow) == 0, f"flow_conservation_transit_{k}_{r_idx}_{v}"
...
for r_idx, r in enumerate(R) :
    for k in range(1, r['|kr|'] + 1):  # Duyệt qua tất cả các đường dẫn của yêu cầu r
        for z_idx, z in enumerate(Z):  # Duyệt qua tất cả các khu vực thảm họa
            ...
            mdl += alpha_krz[k, r_idx, z_idx] <= lpSum(p_kra[k, r_idx, a_idx] for a_idx, a in enumerate(z)), f"disaster_zone_constraint6_{k}_{r_idx}_{z_idx}"
            
            # Ràng buộc (7): α_{k, r, z} ≥ p_{k, r, a}, ∀a ∈ z
            for a_idx, a in enumerate(z):
                mdl += alpha_krz[k, r_idx, z_idx] >= p_kra[k, r_idx, a_idx], f"disaster_zone_constraint7_{k}_{r_idx}_{z_idx}_{a_idx}"
    ...
    for k in range(1, r['|kr|'] + 1):  # Duyệt qua các đường dẫn của yêu cầu r
        for z_idx, z in enumerate(Z):
            if z != zr:  # Kiểm tra xem khu vực thảm họa z có phải là z_r không
                ...
                mdl += lpSum((alpha_krz[k, r_idx, z_idx] for k in range(1, r['|kr|'] + 1))) <= 1, f"disaster_zone_disjoint_constraint_{k}_{r_idx}_{z_idx}"
...
for r_idx, r in enumerate(R):
    for m_idx, m in enumerate(M):
        for k in range(1, r['|kr|'] + 1):
            mdl += lpSum(a[2] * p_kra[k, r_idx, a_idx] for a_idx, a in enumerate(A)) <= m['hm'] + h_max * (1 - b_kmr[k, m_idx, r_idx]), f"modulation_format_selection_{k}_{r_idx}_{m_idx}"


# Ràng buộc (10): Ensure only one modulation format is selected for each path
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        mdl += lpSum(b_kmr[k, m_idx, r_idx] for m_idx, m in enumerate(M)) <= w_kr[k, r_idx], f"one_modulation_format_per_path_{k}_{r_idx}"


# Ràng buộc (11): FS assigned for each request
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        mdl += phi_kr[k, r_idx] == lpSum(phi_krm[k, r_idx, m_idx] for m_idx, m in enumerate(M)), f"fs_assigned_{k}_{r_idx}"
...
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        for m_idx, m in enumerate(M):
            mdl += phi_krm[k, r_idx, m_idx] <= b_kmr[k, m_idx, r_idx] * len(S), f"fs_assignment_constraint_{k}_{r_idx}_{m_idx}"
...
for r_idx, r in enumerate(R):
    for a_idx, a in enumerate(A):
        for k in range(1, r['|kr|'] + 1):
            mdl += p_kra[k, r_idx, a_idx] <= w_kr[k, r_idx],f"path_selection_constraint_{k}_{r_idx}_{a_idx}"


# Ràng buộc (14): Số lượng đường dẫn hoạt động cho mỗi yêu cầu
for r_idx, r in enumerate(R):
    mdl += lpSum(w_kr[i, r_idx] for i in range(1, r['|kr|'])) == lpSum(i * Xi_ir[i, r_idx] for i in range(1, r['|kr|'])), f"number_of_working_paths_{r_idx}"


# Ràng buộc (15): Đảm bảo chỉ một số đường dẫn hoạt động được chọn
for r_idx, r in enumerate(R):
    mdl += lpSum(Xi_ir[i, r_idx] for i in range(1, r['|kr|'])) == 1, f"single_working_path_selection_{r_idx}"
...
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        mdl += lpSum(phi_krm[k, r_idx, m_idx] * m['Tm'] for m_idx, m in enumerate(M))+ (1 - w_kr[k, r_idx]) * r['φr'] >= r['φr'] * lpSum((Xi_ir[i, r_idx] * (1/i)) for i in range(1, r['|kr|'])),f"fs_assignment_sufficient_{k}_{r_idx}"        
...
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        for a_idx, a in enumerate(A):
            mdl += phi_kra[k, r_idx, a_idx] <= p_kra[k, r_idx, a_idx] * len(S), f"fs_limit_on_link_{k}_{r_idx}_{a_idx}"
...
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        for a_idx, a in enumerate(A):
            mdl += phi_kra[k, r_idx, a_idx] <= phi_kr[k, r_idx], f"fs_link_less_than_path_{k}_{r_idx}_{a_idx}"
...
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        for a_idx, a in enumerate(A):
            mdl += phi_kra[k, r_idx, a_idx] >= phi_kr[k, r_idx] - len(S) * (1 - p_kra[k, r_idx, a_idx]),f"fs_link_equals_path_if_selected_{k}_{r_idx}_{a_idx}"
    ...
    for a_idx, a in enumerate(A):
        for k in range(1, r['|kr|'] + 1):
            for kp in range(1, r['|kr|'] + 1):
                if k > kp:
                    mdl += p_kra[k, r_idx, a_idx] + p_kra[kp, r_idx, a_idx] - 1 <= gamma_kkp_r[k, kp, r_idx], f"shared_link_same_request_{k}_{kp}_{r_idx}_{a_idx}"
...
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        for kp in range(1, r['|kr|'] + 1):
            if k > kp:
                mdl += gamma_kkp_r[k, kp, r_idx] == gamma_kkp_r[kp, k, r_idx], f"symmetric_gamma_same_request_{k}_{kp}_{r_idx}"
...
for r_idx, r in enumerate(R):
    for k in range(1, r['|kr|'] + 1):
        for kp in range(1, r['|kr|'] + 1):
            if k > kp:
                mdl += beta_kkp_r[k, kp, r_idx] + beta_kkp_r[kp, k, r_idx] == 1, f"compare_fs_same_request_{k}_{kp}_{r_idx}"
...
for r_idx, r in enumerate(R):
    for rp_idx, rp in enumerate(R):
        if r_idx > rp_idx:
            for k in range(1, r['|kr|'] + 1):
                for kp in range(1, rp['|kr|'] + 1):
                    mdl += beta_kkp_rrp[k, kp, r_idx, rp_idx] + beta_kkp_rrp[kp, k, rp_idx, r_idx] == 1, f"compare_fs_diff_request_{k}_{kp}_{r_idx}_{rp_idx}"
            ...
            for k in range(1, r['|kr|'] + 1):
                for kp in range(1, rp['|kr|'] + 1):
                    mdl += (
                        ...
                        <= M * (2 - gamma_kkp_rrp[k, kp, r_idx, rp_idx] - beta_kkp_rrp[k, kp, r_idx, rp_idx]), 
...
theta_1 = 1
theta_2 = 1
objective = theta_1 * lpSum(phi_kra[k, r_idx, a_idx]
                            for r_idx, r in enumerate(R)
                            for a_idx, a in enumerate(A)
                            for k in range(1, r['|kr|'] + 1)) + theta_2 * MOFI
mdl += objective
...
for r_idx, r in enumerate(R):
    ...
    for k in range(1, r['|kr|'] + 1):
        selected_links = [A[a_idx] for a_idx, a in enumerate(A) if value(p_kra[k, r_idx, a_idx]) == 1]
...
if mdl.status == 1:
    ...
    for r_idx, r in enumerate(R):
        ...
        nx.draw_networkx_edges(G, pos, edgelist=all_edges, edge_color=edge_colors, arrows=True)
