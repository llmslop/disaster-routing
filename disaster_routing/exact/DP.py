...
# Ví dụ sử dụng:
num_requests = 3  # Số lượng yêu cầu cần tạo
...
for r_idx, r in enumerate(R):
    for c in C:
        ...
        mdl += (
            lpSum(R_crd[c, r_idx, d] for d in D) >= 2,
            f"content_storage_lower_bound_{c}_{r_idx}",
        )
        mdl += (
            lpSum(R_crd[c, r_idx, d] for d in D) <= r["|kr|"],
            f"content_storage_upper_bound_{c}_{r_idx}",
        )
...
for r_idx, r in enumerate(R):
    zr = r["zr"]  # DZ của nút nguồn
    for z_idx, z in enumerate(Z):
        if z != zr:  # Loại trừ DZ của nút nguồn
            for k in range(1, r["|kr|"] + 1):
                mdl += (
...
for r_idx, r in enumerate(R):
    for k in range(1, r["|kr|"] + 1):
        ...
        lhs = (
            lpSum(
                phi_krm[k, r_idx, m_idx] * M[m_idx]["Tm"] for m_idx, m in enumerate(M)
            )
            + (1 - w_kr[k, r_idx]) * r["φr"]
        )

        # Right-hand side of the constraint (reformulated to avoid division)
        rhs = r["φr"] * lpSum(Xi_ir[i, r_idx] for i in range(1, r["|kr|"]))

        # Add the constraint
        mdl += lhs >= rhs, f"fs_assignment_sufficient_{k}_{r_idx}"
...
# --- Hàm mục tiêu ---
# Objective function
theta_1 = 1  # Trọng số cho tổng mức sử dụng phổ
theta_2 = 1  # Trọng số cho MOFI (điều chỉnh nếu cần)
objective = (
    theta_1
    * lpSum(
        phi_kra[k, r_idx, a_idx]
