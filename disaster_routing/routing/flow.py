    ...
    def route_request(self, req: Request, top: Topology, dst: list[int]) -> list[Route]:
        ...
        for K in range(1, len(dst) + 1):
            ...
            for i, dz in enumerate(top.dzs):
                name = f"DZ{i}"
                flow_graph.add_node(name, demand=0)
                if req.source in dz.nodes:
                    flow_graph.add_edge("source", name)
                if len(dz.nodes.intersection(dst)) > 0:
                    flow_graph.add_edge(name, "sink", capacity=1)
            ...
            for i, dzi in enumerate(top.dzs):
                for j in range(i + 1, len(top.dzs)):
                    ...
                    if any(
                        ...
                        flow_graph.add_edge(f"DZ{i}", f"DZ{j}", weight=1, capacity=1)
                        flow_graph.add_edge(f"DZ{j}", f"DZ{i}", weight=1, capacity=1)
            ...
            try:
                ...
                for dz_path in dz_paths:
                    dz_set_list = [top.dzs[int(i[2:])].nodes for i in dz_path[1:-1]]
